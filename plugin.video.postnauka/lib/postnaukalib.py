...
class Web:
    ...
    def get_url(self, url):
        ...
        self.log.debug("URL {url} was retrieved successfully!".format(
            url=self.url))
        dump("link_dumped_" + url.split("/")[-1], content)
        return content
...
def kodi(sort=None):
    log = Logger()
    log.debug("Building menu with sort {}".format(repr(sort)))

    def deca(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):

            log.debug("Building decorator with sort {}".format(repr(sort)))
            for item in func(*args, **kwargs):
                log.debug("Decorator Building item {}".format(repr(item)))
...
class List:
    ...
    def main_menu(self):
        ...
        # kodi_build(menu_items, sort=xbmcplugin.SORT_METHOD_LABEL_IGNORE_THE)
